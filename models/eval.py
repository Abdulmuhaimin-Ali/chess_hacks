...
###############################################
# Play ONE game vs Stockfish at level
###############################################
def play_game_at_level(model, sf_path, sf_level, model_white, sims, device):
    board = chess.Board()
    engine = chess.engine.SimpleEngine.popen_uci(sf_path)

    # Hard-code: full-strength SF, we weaken only via nodes.
    engine.configure({"Threads": 1})
    engine.configure({"Skill Level": 20})   # max strength, no Elo limiting
    engine.configure({"UCI_LimitStrength": False})

    # Make levels VERY different
    nodes_map = {
        1: 50,
        2: 200,
        3: 800,
    ...
    nodes = nodes_map[sf_level]
    print(f"[INFO] SF level {sf_level}: nodes = {nodes}")
